...
export default function Login({ onLogin, onClose, modal = false, onOpenAccount }: LoginProps) {
  ...
  const formContent = (
    <div style={{
      ...
      <h2 style={{ color: '#fff', fontWeight: 700, fontSize: '1.6rem', marginBottom: 6, textAlign: 'center' }}>Welcome back</h2>
      <p style={{ color: '#556', fontSize: '0.88rem', textAlign: 'center', marginBottom: '1.5rem' }}>Sign in to your private banking dashboard</p>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        <div>
          ...
          <input
            ...
            onBlur={e => (e.target.style.borderColor = 'rgba(196,160,82,0.1)')}
          />
        </div>
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 7 }}>
            ...
            <span style={{ fontSize: '0.78rem', color: 'rgba(196,160,82,0.55)', cursor: 'pointer', transition: 'color 0.2s' }}
              ...
              onMouseOut={e => (e.currentTarget.style.color = 'rgba(196,160,82,0.55)')}
        ...
        {error && (
          <div style={{
            ...
            borderRadius: 8, padding: '0.7rem 1rem', color: '#ff7875', fontSize: '0.85rem',
        ...
        <button type="submit" disabled={loading} style={{
          ...
          color: '#060913', border: 'none', borderRadius: 10,
          padding: '0.9rem', fontWeight: 700, fontSize: '0.95rem', cursor: loading ? 'not-allowed' : 'pointer',
  ...
  return (
    <main style={{
      background: '#070910', minHeight: '100vh',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      padding: '2rem',
      backgroundImage: 'radial-gradient(ellipse 60% 50% at 50% 0%, rgba(196,160,82,0.05) 0%, transparent 70%)',
    }}>
      {formContent}
    </main>
  );
